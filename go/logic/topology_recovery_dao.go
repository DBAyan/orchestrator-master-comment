...
// AttemptRecoveryRegistration tries to add a recovery entry; if this fails that means recovery is already in place. 尝试添加一个恢复条目；如果这一尝试失败，那么意味着恢复已经在进行中。
func AttemptRecoveryRegistration(analysisEntry *inst.ReplicationAnalysis, failIfFailedInstanceInActiveRecovery bool, failIfClusterInActiveRecovery bool) (*TopologyRecovery, error) {
	if failIfFailedInstanceInActiveRecovery { // true
		...
		// 具体而言，它检查当前实例是否最近刚刚被提升（升级为某种角色），并且仍处于活跃期。如果是这样，我们拒绝进行恢复注册，以避免频繁的状态变化（flapping）。
	...
	if failIfClusterInActiveRecovery { // true  上一层传入的是同一个参数
		// Let's check if this cluster has just experienced a failover and is still in active period.
		// If so, we reject recovery registration to avoid flapping.
		// 它检查当前集群是否刚刚经历了一次故障转移（failover），并且仍处于活跃期。
		//如果是这样，我们拒绝进行恢复注册，以避免频繁的状态变化（flapping）。
	...
	if !failIfFailedInstanceInActiveRecovery {
		...
		// 这段注释表明代码在某种情况下隐式地确认了该实例可能存在的活跃恢复（active recovery），前提是这些恢复已经完成。
		...
		// 解释了为什么代码只承认已完成的恢复，以解决两个数据库管理员（DBAs）同时尝试在同一时刻恢复相同实例的可能情况。
	}
...
// ExpireBlockedRecoveries clears listing of blocked recoveries that are no longer actually blocked. 清除实际不再被阻止的被阻止恢复的列表。
func ExpireBlockedRecoveries() error {
	...
	// 较早的恢复现在已被确认，因此应该释放阻塞的恢复。
	//请注意，blocked_topology_recovery 中的数据仅用于审核：它不是我们做出自动决策的数据。
...
//函数用于读取给定实例的已完成恢复记录，其中该实例是由于某些原因被提升（升级），并且仍然处于活跃期（active period）。
//这个函数可能用于阻止进一步的恢复，以防止在活跃期内重新进行恢复操作。
...
//  函数用于读取给定实例的已完成恢复记录，其中该实例是由于某些原因被提升（升级），并且仍然处于活跃期（active period）。这个函数可能用于阻止进一步的恢复，以防止在活跃期内重新进行恢复操作。
