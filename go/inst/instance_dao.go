...
// backend. 用于收集关于MySQL服务器状态的信息，并将结果同步写入 orchestrator 后端。
...
// 该操作连接到一个拓扑的 MySQL 实例，收集关于该服务器以及其复制状态的信息，并将检索到的信息写入 orchestrator 的后端。此外，该操作还提供了以下功能：
// 写操作可以选择进行缓冲。
// 可以收集执行各个阶段的时间信息。
func ReadTopologyInstanceBufferable(instanceKey *InstanceKey, bufferWrites bool, latency *stopwatch.NamedStopwatch) (inst *Instance, err error) {
	...
	// 每秒执行函数 UpdateInstanceLastAttemptedCheck 更新 last_attempted_check 字段为当前时间
	lastAttemptedCheckTimer := time.AfterFunc(time.Second, func() {
		go UpdateInstanceLastAttemptedCheck(instanceKey)
	})

	latency.Start("instance")
	// 创建被发现实例的数据库对象，用于一些获取梳理信息的查询，查询之后的数据都会记录到 instance 结构体中
	...
	if isMaxScale {
		...
		partialSuccess = true // We at least managed to read something from the server. 部分成功
		// 处理hostname
		...
		// 如果该实例是 oracle 或者 percona的MySQL ，并且该实例的主版本大于 5.6
	...
	// 从库相关的信息 show  slave status，如果可以执行该命令 ，还能获取到master instance key
	...
	if config.Config.ReplicationLagQuery != "" && !isMaxScale {
		...
		go func() {
			...
			if err := db.QueryRow(config.Config.ReplicationLagQuery).Scan(&instance.ReplicationLagSeconds); err == nil {
				...
				// 如果查询有错误 则使用SecondsBehindMaster
				instance.ReplicationLagSeconds = instance.SecondsBehindMaster
	...
	// -------------------------------------------------------------------------
	// Anything after this point does not affect the fact the instance is found. 该时间点之后的任何内容都不会影响找到实例的事实。
	// No `goto Cleanup` after this point. 该时间点之后 不会再运行 goto Cleanup
	// -------------------------------------------------------------------------

	// Get replicas, either by SHOW SLAVE HOSTS or via PROCESSLIST 获取该实例的从副本 ，通过 SHOW SLAVE HOSTS 或者 PROCESSLIS
	// MaxScale does not support PROCESSLIST, so SHOW SLAVE HOSTS is the only option 。MaxScale 不支持 PROCESSLIST，所以 只能通过SHOW SLAVE HOSTS
	// 通过 ShowSlaveHosts  发现从库
	...
	// 通过DetectDataCenterQuery自定义的查询 会覆盖参数 DataCenterPattern 获取到的值
	...
	// First read the current PromotionRule from candidate_database_instance.  通过candidate_database_instance 读取 当前的选主优先级
	{
		latency.Start("backend")
	...
	// time an instance is discovered, and setting a rule like "must_not".  // 然后检查实例是否要设置不同的 PromotionRule。
	//  我们将代表他们在此处通过  DetectPromotionRuleQuery 设置它，这样在第一次发现实例和设置“must_not”之类的规则之间就不会出现竞争。
	if config.Config.DetectPromotionRuleQuery != "" && !isMaxScale {
		...
		go func() {
			...
			if err == nil {
				// We need to update candidate_database_instance. 我们需要更新表candidate_database_instance ，及时没有变化
	...
	if !isMaxScale {
		if instance.SuggestedClusterAlias == "" {
			// Only need to do on masters // 只能在主库查询
...
// ReadClusterAliasOverride reads and applies SuggestedClusterAlias based on cluster_alias_override // 根据 cluster_alias_override 读取并应用 SuggestedClusterAlias
...
// and getting it from there. 通过访问该实例所在集群的master 获取 集群名字
...
func ReadInstanceClusterAttributes(instance *Instance) (err error) {
	...
	// Read the cluster_name of the _master_ or _group_primary_ of our instance, derive it from there. 通过查询表database_instance 条件为该实例的master获取 集群名字
	query := `
			select
					cluster_name,
	...
	if clusterName == "" {
		// Nothing from master; we set it to be named after the instance itself  没有从master上获取到集群的名字，我们将以实例本身命名
...
// readInstancesByCondition 是一个用于从后端数据库读取实例的通用函数。
	...
	readFunc := func() ([](*Instance), error) {
		...
		select
			*,
			...
			unix_timestamp() - unix_timestamp(last_seen) as x,
...
// ReadInstance reads an instance from the orchestrator backend database 从 Orchestrator 后端数据库读取实例
func ReadInstance(instanceKey *InstanceKey) (*Instance, bool, error) {
	...
	// And we expect to find one // 我们知道最多有一个（主机名和端口是 PK）并且我们期望找到一个
...
// ReadClusterMaster returns the master of this cluster. 返回集群的主库
...
// ReadReplicaInstances reads replicas of a given master 根据主读取从副本
...
// ReadClusterCandidateInstances reads cluster instances which are also marked as candidates 读取集群中被标记为候选节点的实例信息
...
// the instance.
// ReadOutdatedInstanceKeys 读取并返回所有不是最新的instanceKey（即自上次检查以来已经过了预配置的时间）
//  但我们也会检查尝试进行实例检查但未进行实际检查的情况！ 当 TCP/IP 连接挂起时，可能会发生这种情况，在这种情况下，“检查”永远不会返回。 在这种情况下，我们将间隔乘以一个因子，以免在实例上打开太多连接。
...
// WriteInstance stores an instance in the orchestrator backend 将该实例的信息存储到后端数据库
...
//  这个函数的作用是更新 Orchestrator 后端数据库中给定实例的 last_check 时间戳。last_check 时间戳通常用于跟踪实例的最后一次检查时间，以便了解实例的健康状态和最新的状态。
...
// UpdateInstanceLastAttemptedCheck 更新给定实例的OC后端数据库中的last_attempted_check时间戳。
//这用作故障安全机制，以防对实例的访问挂起（如果发生），在这种情况下，整个 ReadTopology 被卡住（不，连接超时对驱动程序超时也没有帮助。别看我，这个世界是一个严酷的居住地）。
//因此，我们在尝试访问实例之前一定要记下； 当我们希望再次访问该实例时，这会引发一个危险信号：如果last_attempted_check比last_checked*新*，这是个坏消息，意味着我们遇到了“悬而未决”的问题。
