...
// SQLThreadUpToDate returns true when the instance had consumed all relay logs. 读取到的主库的binlog位点  与 从库执行到的主库binlog位点是否相等
...
func (this *Instance) CanReplicateFrom(other *Instance) (bool, error) {
	...
	if this.IsSmallerMajorVersion(other) && !this.IsBinlogServer() {// 检查版本 和 都否为binlog server
	...
	if this.LogBinEnabled && this.LogReplicationUpdatesEnabled {
		if this.IsSmallerBinlogFormat(other) {// 检查binlog格式
