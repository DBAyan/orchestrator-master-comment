...
func shouldPostponeRelocatingReplica(replica *Instance, postponedFunctionsContainer *PostponedFunctionsContainer) bool {
	...
	if config.Config.PostponeReplicaRecoveryOnLagMinutes > 0 && // 设置了参数PostponeReplicaRecoveryOnLagMinutes
		replica.SQLDelay > config.Config.PostponeReplicaRecoveryOnLagMinutes*60 { // 该实例的延迟大于参数PostponeReplicaRecoveryOnLagMinutes 设置的值
		...
		// 即某个从副本（replica）的延迟非常严重，同时系统配置了推迟在这个复制上执行操作，以防止拖慢其他操作的进行。
		return true
	}
...
func canReplicateAssumingOracleGTID(instance, masterInstance *Instance) (canReplicate bool, err error) {
	subtract, err := GTIDSubtract(&instance.Key, masterInstance.GtidPurged, instance.ExecutedGtidSet)// 主库的gtid_purged 减去 从副本的 gtid_executed
	if err != nil {
		return false, err
	}
	...
	return subtractGtidSet.IsEmpty(), nil // 空的话返回true
}
...
func CheckMoveViaGTID(instance, otherInstance *Instance) (err error) {
	isOracleGTID, _, moveCompatible := instancesAreGTIDAndCompatible(instance, otherInstance) // 两个实例数据类型需要兼容
	if !moveCompatible {
...
// 尝试将一个给定的实例移动到另一个实例下面的函数。这个移动操作使用了 Oracle GTID 或 MariaDB GTID。
func moveInstanceBelowViaGTID(instance, otherInstance *Instance) (*Instance, error) {
	rinstance, _, _ := ReadInstance(&instance.Key) // key 有效
	if canMove, merr := rinstance.CanMoveViaMatch(); !canMove { // 最近检查有效
		return instance, merr
	}

	if canReplicate, err := instance.CanReplicateFrom(otherInstance); !canReplicate { //  可以作为从库
		return instance, err
	}
	if err := CheckMoveViaGTID(instance, otherInstance); err != nil { // gtid 集合复合要求
		return instance, err
	}
	...
	var err error
	if maintenanceToken, merr := BeginMaintenance(instanceKey, GetMaintenanceOwner(), fmt.Sprintf("move below %+v", *otherInstanceKey)); merr != nil { // 设置维护状态
		err = fmt.Errorf("Cannot begin maintenance on %+v: %v", *instanceKey, merr)
		goto Cleanup
	} else {
		defer EndMaintenance(maintenanceToken) // 结束维护状态
	}
...
// that could not be moved (do not use GTID or had GTID errors) ，其主要功能是通过 GTID 将一组副本移到另一个实例下，并返回无法移动的副本列表（不使用 GTID 或出现 GTID 错误的副本）。
func moveReplicasViaGTID(replicas [](*Instance), other *Instance, postponedFunctionsContainer *PostponedFunctionsContainer) (movedReplicas [](*Instance), unmovedReplicas [](*Instance), err error, errs []error) {
	replicas = RemoveNilInstances(replicas) // 从可移动的从副本列表中移除所有值为 nil 的实例。
	replicas = RemoveInstance(replicas, &other.Key) // 从可移动的从副本中 移除 候选主
	if len(replicas) == 0 { // 如果经过处理可移动从副本个数为0 不做然和处理
		// Nothing to do
	...
	for _, replica := range replicas { // 遍历可移动从副本
		replica := replica

		waitGroup.Add(1)
		// Parallelize repoints 并行化执行
		go func() {
			...
			moveFunc := func() error {
				...
				movedReplica, replicaErr := moveInstanceBelowViaGTID(replica, other) // 传入的参数 replicasToMove, candidateReplica
				...
				if replicaErr == nil {
					movedReplicas = append(movedReplicas, replica) // 追加可移动的实例列表
				} else {
					unmovedReplicas = append(unmovedReplicas, replica) // 追加不可移动的实例列表
					errs = append(errs, replicaErr)
				}
				return replicaErr
			}
			if shouldPostponeRelocatingReplica(replica, postponedFunctionsContainer) {// 延迟建立主从
				...
				ExecuteOnTopology(func() { moveFunc() }) // 执行匿名函数 ，将位点相等，位点落后且满足要求可以成为从副本的实例，变为候选主的从副本
			}
		}()
	}
	waitGroup.Wait()
...
// DetachReplicaMasterHost detaches a replica from its master by corrupting the Master_Host (in such way that is reversible) 用于将从库（replica）与其主库（master）分离
...
// sortInstances 根据某种逻辑对给定的实例列表进行洗牌（shuffles）。
func sortInstancesDataCenterHint(instances [](*Instance), dataCenterHint string) {
	...
	// Less 方法是 sort.Interface 接口的一部分，该接口用于在排序时比较元素的大小。
	// 在使用 Go 语言的标准库中的排序函数时（例如 sort.Sort），会调用 sort.Interface 接口的 Len、Less 和 Swap 方法。
}
...
// getReplicasForSorting 返回给定主节点的副本列表，可能用于选择候选副本。
...
// sortedReplicas 返回某个主节点的副本列表，按照执行坐标（exec coordinates）排序（最新的副本排在前面）。这个函数假设给定的 replicas 参数确实是从同一个主节点复制的实例列表
...
func isGenerallyValidAsCandidateReplica(replica *Instance) bool {
	...
	if replica.IsBinlogServer() {
		...
		// 无法在 binlog 服务器下重新分组，因为它不支持伪 GTID 相关的查询，比如 SHOW BINLOG EVENTS。
		return false
	}
...
//  返回在给定实例中找到的主要（最常见）主版本号。这将用于选择升级的最佳候选项。
...
// among given instances. This will be used for choosing best candidate for promotion.
// 用于获取在给定的副本实例列表中具有最高优先级的 binlog 格式。
...
func chooseCandidateReplica(replicas [](*Instance)) (candidateReplica *Instance, aheadReplicas, equalReplicas, laterReplicas, cannotReplicateReplicas [](*Instance), err error) {
	...
	// 按照副本中主要版本号的出现次数降序排序，并返回出现次数最多的主要版本号
	...
	for _, replica := range replicas {
		replica := replica
		if isGenerallyValidAsCandidateReplica(replica) && // ，binlog已经打开，参数打开，不是binlogServer
			!IsBannedFromBeingCandidateReplica(replica) && // 没有被被禁止成为候选主
			!IsSmallerMajorVersion(priorityMajorVersion, replica.MajorVersionString()) && // 从副本的主版本号不能小于 priorityMajorVersion
			!IsSmallerBinlogFormat(priorityBinlogFormat, replica.Binlog_format) { // 从副本的binlog格式要大于 priorityBinlogFormat row > mixed> statement
			// this is the one
			candidateReplica = replica // 该从副本可以成为候选主
			break
		}
	}
	if candidateReplica == nil { // 如果没有找到候选主
		// Unable to find a candidate that will master others.
		// Instead, pick a (single) replica which is not banned.
		// 描述了一种无法找到适当的主节点候选的情况。在这种情况下，代替方案是选择一个（单个的）副本，该副本尚未被禁止成为候选主。
		for _, replica := range replicas {
			replica := replica
		...
		if candidateReplica != nil { // 如果第二次找到了候选主，从replicas列表中删除这个key
			replicas = RemoveInstance(replicas, &candidateReplica.Key)
		}
		// 经过第二次选候选从副本还是没有选择出来，则函数结束了
	...
	// 如果第一次就找到了候选主 ，从replicas列表中删除这个key
	replicas = RemoveInstance(replicas, &candidateReplica.Key)
	// 遍历所有其他的从库 除了候选主
	for _, replica := range replicas {
		replica := replica
		if canReplicate, err := replica.CanReplicateFrom(candidateReplica); !canReplicate {
			...
			cannotReplicateReplicas = append(cannotReplicateReplicas, replica) // 不能作为从副本的实例
			if err != nil {
			...
			laterReplicas = append(laterReplicas, replica) // binlog位点 落后于 候选主的binlog位点，比较的是
		} else if replica.ExecBinlogCoordinates.Equals(&candidateReplica.ExecBinlogCoordinates) {
			equalReplicas = append(equalReplicas, replica) // binlog位点 等于 候选主的binlog位点，比较的是
		} else {
			// lost due to being more advanced/ahead of chosen replica.
			aheadReplicas = append(aheadReplicas, replica)// binlog位点 领先于 候选主的binlog位点，比较的是
		}
	}
	return candidateReplica, aheadReplicas, equalReplicas, laterReplicas, cannotReplicateReplicas, err
}

// GetCandidateReplica chooses the best replica to promote given a (possibly dead) master 对于给定的master  选择一个最合适的从副本用于提升
func GetCandidateReplica(masterKey *InstanceKey, forRematchPurposes bool) (*Instance, [](*Instance), [](*Instance), [](*Instance), [](*Instance), error) {
	var candidateReplica *Instance // 候选的从副本
	aheadReplicas := [](*Instance){} // 超前的从副本
	equalReplicas := [](*Instance){} // 相同的从副本
	laterReplicas := [](*Instance){} // 落后的从副本
	cannotReplicateReplicas := [](*Instance){} // 不能复制的从副本
	...
	if forRematchPurposes { // 传入的是 true
		stopReplicationMethod = StopReplicationNice
	}
	//  根据SQL获取该主库的所有从副本  ，停止从副本，并进行排序操作
	replicas = sortedReplicasDataCenterHint(replicas, stopReplicationMethod, dataCenterHint) //  stopReplicationMethod = StopReplicationNice
	// 如果没有从副本
	if len(replicas) == 0 {
	...
	if candidateReplica != nil { // 候选主不为空
		mostUpToDateReplica := replicas[0] // 得到执行binlog最新的从库，这里replicas 在上面已经按执行位点排序了
		if candidateReplica.ExecBinlogCoordinates.SmallerThan(&mostUpToDateReplica.ExecBinlogCoordinates) {// 如果候选主没有最新的从的执行位点多，则发出警告
...
// RegroupReplicasGTID 将选择给定实例的候选副本，并使用 GTID（全局事务标识）接管其他副本。
// 接收 6个参数  返回5个参数
func RegroupReplicasGTID(
	masterKey *InstanceKey, // failedInstanceKey
	returnReplicaEvenOnFailureToRegroup bool, // true
	startReplicationOnCandidate bool,  //false
	onCandidateReplicaChosen func(*Instance), // nil
	postponedFunctionsContainer *PostponedFunctionsContainer,
	postponeAllMatchOperations func(*Instance, bool) bool,  // 是在上一层中定义的匿名函数
) (
	lostReplicas [](*Instance), // 丢弃的从副本
	movedReplicas [](*Instance), // 被移动的从副本
	cannotReplicateReplicas [](*Instance), // 不能复制的从副本
	candidateReplica *Instance, // 候选从副本
	err error,
) {
	var emptyReplicas [](*Instance) // 空的从副本
	var unmovedReplicas [](*Instance) // 没有移动的从副本
	candidateReplica, aheadReplicas, equalReplicas, laterReplicas, cannotReplicateReplicas, err := GetCandidateReplica(masterKey, true)
	if err != nil {
		if !returnReplicaEvenOnFailureToRegroup {
			candidateReplica = nil
	...
	// 将位点相等的 ，位点落后的从副本 归为可以移动的从副本
	replicasToMove := append(equalReplicas, laterReplicas...)
	hasBestPromotionRule := true // 候选主有最优的提升规则标志 设置为true
	if candidateReplica != nil { // 候选主 不为空
		for _, replica := range replicasToMove { // 遍历可移动的从副本
			if replica.PromotionRule.BetterThan(candidateReplica.PromotionRule) { // 比较可移动副本列表 与 候选主的提升规则 ，查看是否有优于候选主的替身规则，提升规则定义为常量 ，可比较
				hasBestPromotionRule = false // 候选主有最优的提升规则标志 设置为false
			}
		}
	}
	// 匿名函数 moveGTIDFunc
	moveGTIDFunc := func() error {
		...
		unmovedReplicas = append(unmovedReplicas, aheadReplicas...) // 将位点多的从副本 加入到 不可移动的从副本
		return log.Errore(err)
	}
...
// binlog-position, pseudo-gtid, repointing, binlog servers...
// 该函数将尝试将由 instanceKey 下的从副本移动到另一个otherKey 实例之下。
// Orchestrator 寻找最佳方式： Orchestrator 将尝试找出最佳的方式来重新定位这些服务器。这可能涉及到正常的二进制日志位置、伪 GTID、重新指向、二进制日志服务器等操作。
func RelocateReplicas(instanceKey, otherKey *InstanceKey, pattern string) (replicas [](*Instance), other *Instance, err error, errs []error) {
	...
	instance, found, err := ReadInstance(instanceKey) // 读取老主的实例信息
	if err != nil || !found {
		return replicas, other, log.Errorf("Error reading %+v", *instanceKey), errs
	}
	other, found, err = ReadInstance(otherKey) // 读取新主的实例信息
	if err != nil || !found {
		return replicas, other, log.Errorf("Error reading %+v", *otherKey), errs
	}

	replicas, err = ReadReplicaInstances(instanceKey) // 读取老主的所有从副本
	if err != nil {
		return replicas, other, err, errs
	}
	replicas = RemoveInstance(replicas, otherKey) // 从所有从副本中 移除 新主
	replicas = filterInstancesByPattern(replicas, pattern)
	if len(replicas) == 0 { // 从副本个数为0
		// Nothing to do
		return replicas, other, nil, errs
	}
	for _, replica := range replicas { // 从副本个数不为0 ，遍历所有从副本
	...
	replicas, err, errs = relocateReplicasInternal(replicas, instance, other) // 重新设置主从关系
