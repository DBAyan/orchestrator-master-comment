...
// 这个函数用于计算在将实例视为无效之前，last_attempted_check 到 last_checked 之间允许的最大经过时间。
//具体来说，它返回了配置中的 InstancePollSeconds 和 ReasonableInstanceCheckSeconds 之和。
func ValidSecondsFromSeenToLastAttemptedCheck() uint {
	return config.Config.InstancePollSeconds + config.Config.ReasonableInstanceCheckSeconds // 5+1
}
