...
//该函数用于在拓扑服务器上执行给定的函数，并且在执行期间会维护并发限制，以确保不会泄漏令牌。
...
func StopReplicationNicely(instanceKey *InstanceKey, timeout time.Duration) (*Instance, error) {
	...
	// 如果 SQL Delay 为 0(即不是延迟从库)，等待 SQL Thread 达到 up-to-date 状态
...
func WaitForSQLThreadUpToDate(instanceKey *InstanceKey, overallTimeout time.Duration, staleCoordinatesTimeout time.Duration) (instance *Instance, err error) {
	// Otherwise we don't bother.
	// 定义变量 为 BinlogCoordinates 格式
	var lastExecBinlogCoordinates BinlogCoordinates

	//  如果变量为0 ，则取24小时。整体超时时间
	if overallTimeout == 0 {
		overallTimeout = 24 * time.Hour
	}
	// 如果变量 为 0 ，则取 ReasonableReplicationLagSeconds。老binlog 坐标超时时间
	...
	// 一个for 循环
	for {
		// 调用了 RetryInstanceFunction 函数，并传入了一个匿名函数作为参数。这个匿名函数调用了 ReadTopologyInstance(instanceKey) 函数来读取拓扑实例。
		// 即 RetryInstanceFunction 会多次执行 ReadTopologyInstance 函数
		...
		// 如果  读取到的主库的binlog位点  与 从库执行到的主库binlog位点是否相等 ，则说明sql thread无落后 。
		if instance.SQLThreadUpToDate() {
			// Woohoo
			return instance, nil
		}
		 // 如果 SQLDelay 不为0 ，则说明 可能是延迟从库，返回该实例 和 报错
		if instance.SQLDelay != 0 {
		...
		default:
			...
			time.Sleep(retryInterval) // 休眠0.5秒
		}
	}
}
...
// StopReplicas 会并发地停止给定一组副本的复制。它可能什么都不做，或者尝试 nicely（优雅地）停止，或者正常停止，具体取决于 stopReplicationMethod 参数。
func StopReplicas(replicas [](*Instance), stopReplicationMethod StopReplicationMethod, timeout time.Duration) [](*Instance) {
	if stopReplicationMethod == NoStopReplication { // 不成立
		return replicas
	}
...
// StartReplication starts replication on a given instance. 启动复制
...
// MaybeDisableSemiSyncMaster总是在半同步优先级> 0的情况下禁用半同步主（rpl_semi_sync_master_enabled）。这有点奇怪，但符合传统行为，当启动复制时，我们确实应该禁用副本的半同步主标志。
...
func MaybeDisableSemiSyncMaster(replicaInstance *Instance) (*Instance, error) {
	if replicaInstance.SemiSyncPriority > 0 && replicaInstance.SemiSyncMasterEnabled {
		...
		replicaInstance, err := SetSemiSyncMaster(&replicaInstance.Key, false) // 关闭半同步复制
		if err != nil {
...
// ReadReplicationCredentials Attempt to read and return replication credentials from the mysql.slave_master_info system table
