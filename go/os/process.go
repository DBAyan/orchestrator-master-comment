...
// CommandRun 执行一个文本作为命令。 我们需要把该命令写到一个临时文件然后调用shell 去执行它。然后这个临时文件会被删除
func CommandRun(commandText string, env []string, arguments ...string) error {
	...
	cmd, shellScript, err := generateShellScript(commandText, env, arguments...) // 生成脚本文件
	defer os.Remove(shellScript) // 延迟删除脚本文件
	if err != nil {
	...
	if err != nil {
		// Did the command fail because of an unsuccessful exit code 检查命令是否因为退出码非零而失败
...
// generateShellScript 基于命令 生成一个临时的 shell 脚本，
